...
func GetOrderListByStatus(status, userID int) (orderList []Order, err error) {
	...
	err = db.DB.Select(&orderList, `select oid, commodity.cid, price*count as price, buyer.name, buyer.phone_num, buyer.address, count from purchase_order, commodity, buyer where purchase_order.cid = commodity.cid and purchase_order.uid = buyer.uid and status = ? and sid = ?`, status, sid)
	if err != nil {
		return
	}
